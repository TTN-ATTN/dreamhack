...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/tomcat-embed-websocket-9.0.75.jar:org/apache/tomcat/websocket/server/WsServerContainer.class */
public class WsServerContainer extends WsWebSocketContainer implements ServerContainer {
    ...
    @Override // javax.websocket.server.ServerContainer
    public void addEndpoint(Class<?> pojo) throws IllegalAccessException, InstantiationException, DeploymentException, IllegalArgumentException, InvocationTargetException {
        addEndpoint(pojo, false);
    }

    void addEndpoint(Class<?> pojo, boolean fromAnnotatedPojo) throws IllegalAccessException, InstantiationException, DeploymentException, IllegalArgumentException, InvocationTargetException {
        if (this.deploymentFailed) {
            throw new DeploymentException(sm.getString("serverContainer.failedDeployment", this.servletContext.getContextPath(), this.servletContext.getVirtualServerName()));
        }
        try {
    ...
    private void registerAuthenticatedSession(WsSession wsSession, String httpSessionId) {
        ...
        if (wsSessions == null) {
            this.authenticatedSessions.putIfAbsent(httpSessionId, ConcurrentHashMap.newKeySet());
    ...
    private static void validateEncoders(Class<? extends Encoder>[] encoders, InstanceManager instanceManager) throws IllegalAccessException, InstantiationException, IllegalArgumentException, DeploymentException, InvocationTargetException {
        for (Class<? extends Encoder> encoder : encoders) {
            if (instanceManager == null) {
                ...
                Encoder instance = (Encoder) instanceManager.newInstance((Class<?>) encoder);
                instanceManager.destroyInstance(instance);
            }
        }
    }

    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/tomcat-embed-websocket-9.0.75.jar:org/apache/tomcat/websocket/server/WsServerContainer$TemplatePathMatch.class */
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/tomcat-embed-websocket-9.0.75.jar:org/apache/tomcat/websocket/server/WsServerContainer$ExactPathMatch.class */
