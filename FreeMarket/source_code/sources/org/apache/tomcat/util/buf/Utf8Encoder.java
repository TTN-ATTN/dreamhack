...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/tomcat-embed-core-9.0.75.jar:org/apache/tomcat/util/buf/Utf8Encoder.class */
    ...
    private CoderResult encodeHasArray(CharBuffer in, ByteBuffer out) {
        ...
        while (x < pos + rem) {
            ...
            if (jchar <= 127) {
                ...
                int i = outPos;
                outPos++;
                bArr[i] = (byte) (jchar & Const.MAX_ARRAY_DIMENSIONS);
                outRemaining--;
            } else if (jchar <= 2047) {
                if (outRemaining < 2) {
                    in.position(x);
