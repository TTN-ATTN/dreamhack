...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/tomcat-embed-core-9.0.75.jar:org/apache/coyote/http2/ByteUtil.class */
class ByteUtil {
    private ByteUtil() {
    }
    ...
    static int get31Bits(byte[] input, int firstByte) {
        return ((input[firstByte] & 127) << 24) + ((input[firstByte + 1] & 255) << 16) + ((input[firstByte + 2] & 255) << 8) + (input[firstByte + 3] & 255);
    }

    static int get31Bits(ByteBuffer input, int firstByte) {
        return ((input.get(firstByte) & 127) << 24) + ((input.get(firstByte + 1) & 255) << 16) + ((input.get(firstByte + 2) & 255) << 8) + (input.get(firstByte + 3) & 255);
    }
    ...
    static void set31Bits(byte[] output, int firstByte, int value) {
        ...
        output[firstByte + 3] = (byte) (value & Const.MAX_ARRAY_DIMENSIONS);
    }
    ...
    static void setTwoBytes(byte[] output, int firstByte, int value) {
        ...
        output[firstByte + 1] = (byte) (value & Const.MAX_ARRAY_DIMENSIONS);
    }
    ...
    static void setThreeBytes(byte[] output, int firstByte, int value) {
        ...
        output[firstByte + 2] = (byte) (value & Const.MAX_ARRAY_DIMENSIONS);
    }
