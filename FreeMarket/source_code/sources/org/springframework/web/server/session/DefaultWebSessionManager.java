...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-web-5.3.27.jar:org/springframework/web/server/session/DefaultWebSessionManager.class */
public class DefaultWebSessionManager implements WebSessionManager {
    ...
    private Mono<WebSession> retrieveSession(ServerWebExchange exchange) {
        Flux fluxFromIterable = Flux.fromIterable(getSessionIdResolver().resolveSessionIds(exchange));
        WebSessionStore webSessionStore = this.sessionStore;
        webSessionStore.getClass();
        return fluxFromIterable.concatMap(webSessionStore::retrieveSession).next();
    }
