...
@FunctionalInterface
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-webmvc-5.3.27.jar:org/springframework/web/servlet/function/HandlerFilterFunction.class */
public interface HandlerFilterFunction<T extends ServerResponse, R extends ServerResponse> {
    ...
    static <T extends ServerResponse> HandlerFilterFunction<T, T> ofErrorHandler(Predicate<Throwable> predicate, BiFunction<Throwable, ServerRequest, T> errorHandler) {
        ...
        return (request, next) -> {
            try {
                ServerResponse serverResponseHandle = next.handle(request);
                if (serverResponseHandle instanceof ErrorHandlingServerResponse) {
                    ((ErrorHandlingServerResponse) serverResponseHandle).addErrorHandler(predicate, errorHandler);
                }
