...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-webmvc-5.3.27.jar:org/springframework/web/servlet/tags/UrlTag.class */
public class UrlTag extends HtmlEscapingAwareTag implements ParamAware {
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-webmvc-5.3.27.jar:org/springframework/web/servlet/tags/UrlTag$UrlType.class */
    ...
    String createUrl() throws JspException {
        ...
        if (this.type == UrlType.CONTEXT_RELATIVE) {
            if (this.context == null) {
                ...
                url.append((CharSequence) this.context, 0, this.context.length() - 1);
            } else {
    ...
    protected String replaceUriTemplateParams(String uri, List<Param> params, Set<String> usedParams) throws JspException {
        ...
        for (Param param : params) {
            ...
            if (uri.contains(template)) {
                ...
                if (value != null) {
                    try {
                        strEncodePath = UriUtils.encodePath(value, encoding);
                    } catch (UnsupportedCharsetException ex) {
                        throw new JspException(ex);
                    }
                } else {
                ...
                uri = StringUtils.replace(str, template, strEncodePath);
            } else {
                ...
                if (uri.contains(template2)) {
                    ...
                    if (value2 != null) {
                        try {
                            strEncodePathSegment = UriUtils.encodePathSegment(value2, encoding);
                        } catch (UnsupportedCharsetException ex2) {
                            throw new JspException(ex2);
                        }
                    } else {
                        strEncodePathSegment = "";
                    }
                    uri = StringUtils.replace(str2, template2, strEncodePathSegment);
                } else {
                    continue;
                }
            }
