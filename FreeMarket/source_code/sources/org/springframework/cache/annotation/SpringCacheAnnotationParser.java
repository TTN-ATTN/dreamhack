...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-context-5.3.27.jar:org/springframework/cache/annotation/SpringCacheAnnotationParser.class */
public class SpringCacheAnnotationParser implements CacheAnnotationParser, Serializable {
    ...
    private Collection<CacheOperation> parseCacheAnnotations(DefaultCacheConfig cachingConfig, AnnotatedElement ae, boolean localOnly) {
        Collection<? extends Annotation> collectionFindAllMergedAnnotations;
        if (localOnly) {
            collectionFindAllMergedAnnotations = AnnotatedElementUtils.getAllMergedAnnotations(ae, CACHE_OPERATION_ANNOTATIONS);
        } else {
            collectionFindAllMergedAnnotations = AnnotatedElementUtils.findAllMergedAnnotations(ae, CACHE_OPERATION_ANNOTATIONS);
        }
        Collection<? extends Annotation> anns = collectionFindAllMergedAnnotations;
        if (anns.isEmpty()) {
            return null;
        }
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-context-5.3.27.jar:org/springframework/cache/annotation/SpringCacheAnnotationParser$DefaultCacheConfig.class */
