...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-boot-2.7.12.jar:org/springframework/boot/logging/logback/DefaultLogbackConfiguration.class */
class DefaultLogbackConfiguration {
    ...
    private void defaults(LogbackConfigurator config) {
        ...
        config.getContext().putProperty(LoggingSystemProperties.CONSOLE_LOG_PATTERN, resolve(config, "${CONSOLE_LOG_PATTERN:-%clr(%d{${LOG_DATEFORMAT_PATTERN:-yyyy-MM-dd HH:mm:ss.SSS}}){faint} %clr(${LOG_LEVEL_PATTERN:-%5p}) %clr(${PID:- }){magenta} %clr(---){faint} %clr([%15.15t]){faint} %clr(%-40.40logger{39}){cyan} %clr(:){faint} %m%n${LOG_EXCEPTION_CONVERSION_WORD:-%wEx}}"));
        String defaultCharset = Charset.defaultCharset().name();
        config.getContext().putProperty(LoggingSystemProperties.CONSOLE_LOG_CHARSET, resolve(config, "${CONSOLE_LOG_CHARSET:-" + defaultCharset + "}"));
        config.getContext().putProperty(LoggingSystemProperties.FILE_LOG_PATTERN, resolve(config, "${FILE_LOG_PATTERN:-%d{${LOG_DATEFORMAT_PATTERN:-yyyy-MM-dd HH:mm:ss.SSS}} ${LOG_LEVEL_PATTERN:-%5p} ${PID:- } --- [%t] %-40.40logger{39} : %m%n${LOG_EXCEPTION_CONVERSION_WORD:-%wEx}}"));
        config.getContext().putProperty(LoggingSystemProperties.FILE_LOG_CHARSET, resolve(config, "${FILE_LOG_CHARSET:-" + defaultCharset + "}"));
