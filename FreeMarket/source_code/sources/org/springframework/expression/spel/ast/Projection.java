...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-expression-5.3.27.jar:org/springframework/expression/spel/ast/Projection.class */
public class Projection extends SpelNodeImpl {
    ...
    @Override // org.springframework.expression.spel.ast.SpelNodeImpl
    protected ValueRef getValueRef(ExpressionState state) throws EvaluationException, NegativeArraySizeException {
        ...
        if ((operand instanceof Iterable) || operandIsArray) {
            ...
            for (Object element : data) {
                try {
                    ...
                    state.enterScope(BeanDefinitionParserDelegate.INDEX_ATTRIBUTE, Integer.valueOf(result2.size()));
    ...
    private Class<?> determineCommonType(@Nullable Class<?> oldType, Class<?> newType) {
        ...
        Class<?> superclass = newType;
        while (true) {
            Class<?> nextType = superclass;
            if (nextType != Object.class) {
                if (nextType.isAssignableFrom(oldType)) {
                    return nextType;
                }
                superclass = nextType.getSuperclass();
            } else {
