...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-aop-5.3.27.jar:org/springframework/aop/framework/ProxyFactoryBean.class */
public class ProxyFactoryBean extends ProxyCreatorSupport implements FactoryBean<Object>, BeanClassLoaderAware, BeanFactoryAware {
    ...
    @Nullable
    public Object getObject() throws AopConfigException, BeansException {
    ...
    private synchronized Object newPrototypeInstance() throws BeansException {
        Class<?> targetClass;
    ...
    private boolean isNamedBeanAnAdvisorOrAdvice(String beanName) throws NoSuchBeanDefinitionException {
        ...
        if (!this.advisorChainInitialized && !ObjectUtils.isEmpty((Object[]) this.interceptorNames)) {
            ...
            for (String name : this.interceptorNames) {
                if (name.endsWith("*")) {
                    ...
                    addGlobalAdvisors((ListableBeanFactory) this.beanFactory, name.substring(0, name.length() - "*".length()));
                } else {
    ...
    private List<Advisor> freshAdvisorChain() throws BeansException {
    ...
    private void addGlobalAdvisors(ListableBeanFactory beanFactory, String prefix) {
        String[] globalAdvisorNames = BeanFactoryUtils.beanNamesForTypeIncludingAncestors(beanFactory, (Class<?>) Advisor.class);
        String[] globalInterceptorNames = BeanFactoryUtils.beanNamesForTypeIncludingAncestors(beanFactory, (Class<?>) Interceptor.class);
        if (globalAdvisorNames.length > 0 || globalInterceptorNames.length > 0) {
            ...
            for (String name2 : globalInterceptorNames) {
                if (name2.startsWith(prefix)) {
                    beans.add(beanFactory.getBean(name2));
                }
            }
    ...
    private TargetSource freshTargetSource() throws BeansException {
        if (this.targetName == null) {
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-aop-5.3.27.jar:org/springframework/aop/framework/ProxyFactoryBean$PrototypePlaceholderAdvisor.class */
