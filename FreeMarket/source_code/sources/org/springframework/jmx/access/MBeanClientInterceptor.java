...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-context-5.3.27.jar:org/springframework/jmx/access/MBeanClientInterceptor.class */
public class MBeanClientInterceptor implements MethodInterceptor, BeanClassLoaderAware, InitializingBean, DisposableBean {
    ...
    private void retrieveMBeanInfo(MBeanServerConnection server) throws MBeanInfoRetrievalException {
        try {
            ...
            throw new MBeanInfoRetrievalException("Unable to read MBean info for bean [ " + this.objectName + "]", ex2);
        } catch (IOException ex3) {
            throw new MBeanInfoRetrievalException("An IOException occurred when communicating with the MBeanServer. It is likely that you are communicating with a remote MBeanServer. Check the inner exception for exact details.", ex3);
        } catch (InstanceNotFoundException ex4) {
            throw new MBeanInfoRetrievalException("Unable to obtain MBean info for bean [" + this.objectName + "]: it is likely that this bean was unregistered during the proxy creation process", ex4);
        } catch (IntrospectionException ex5) {
            throw new MBeanInfoRetrievalException("Unable to obtain MBean info for bean [" + this.objectName + "]", ex5);
        }
    }
    ...
    @Nullable
    protected Object convertResultValueIfNecessary(@Nullable Object result, MethodParameter parameter) throws NoSuchMethodException, SecurityException {
        Class<?> elementType;
        Class<?> elementType2;
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-context-5.3.27.jar:org/springframework/jmx/access/MBeanClientInterceptor$MethodCacheKey.class */
