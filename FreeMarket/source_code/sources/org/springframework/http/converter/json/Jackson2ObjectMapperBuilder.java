...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-web-5.3.27.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder.class */
public class Jackson2ObjectMapperBuilder {
    ...
    public <T extends ObjectMapper> T build() {
        ...
        ObjectMapper objectMapperCreate;
        if (this.createXmlMapper) {
            if (this.defaultUseWrapper != null) {
                objectMapperCreate = new XmlObjectMapperInitializer().create(this.defaultUseWrapper.booleanValue(), this.factory);
            } else {
                objectMapperCreate = new XmlObjectMapperInitializer().create(this.factory);
            }
            objectMapper = objectMapperCreate;
        } else {
            objectMapper = this.factory != null ? new ObjectMapper(this.factory) : new ObjectMapper();
        }
    ...
    public void configure(ObjectMapper objectMapper) {
        ...
        if (this.findModulesViaServiceLoader) {
            ObjectMapper.findModules(this.moduleClassLoader).forEach(module -> {
        ...
        if (this.modules != null) {
            this.modules.forEach(module2 -> {
                registerModule(module2, modulesToRegister);
            });
        }
        if (this.moduleClasses != null) {
            for (Class<? extends Module> moduleClass : this.moduleClasses) {
                registerModule((Module) BeanUtils.instantiateClass(moduleClass), modulesToRegister);
            }
        }
        List<Module> modules = new ArrayList<>();
        Iterator<Module> it = modulesToRegister.values().iterator();
        while (it.hasNext()) {
            List<Module> nestedModules = (List) it.next();
    ...
    private void configureFeature(ObjectMapper objectMapper, Object feature, boolean enabled) {
        ...
        if (feature instanceof JsonGenerator.Feature) {
    ...
    private void registerWellKnownModulesIfAvailable(MultiValueMap<Object, Module> modulesToRegister) {
        try {
            Module jdk8Module = (Module) BeanUtils.instantiateClass(ClassUtils.forName("com.fasterxml.jackson.datatype.jdk8.Jdk8Module", this.moduleClassLoader));
            modulesToRegister.set(jdk8Module.getTypeId(), jdk8Module);
        } catch (ClassNotFoundException e) {
        }
        try {
            Module javaTimeModule = (Module) BeanUtils.instantiateClass(ClassUtils.forName("com.fasterxml.jackson.datatype.jsr310.JavaTimeModule", this.moduleClassLoader));
        ...
        if (ClassUtils.isPresent("org.joda.time.YearMonth", this.moduleClassLoader)) {
            try {
                Module jodaModule = (Module) BeanUtils.instantiateClass(ClassUtils.forName("com.fasterxml.jackson.datatype.joda.JodaModule", this.moduleClassLoader));
        ...
        if (KotlinDetector.isKotlinPresent()) {
            try {
                Module kotlinModule = (Module) BeanUtils.instantiateClass(ClassUtils.forName("com.fasterxml.jackson.module.kotlin.KotlinModule", this.moduleClassLoader));
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-web-5.3.27.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder$XmlObjectMapperInitializer.class */
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-web-5.3.27.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder$SmileFactoryInitializer.class */
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/spring-web-5.3.27.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder$CborFactoryInitializer.class */
