...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/freemarker-2.3.33.jar:freemarker/core/_MessageUtil.class */
public class _MessageUtil {
    ...
    static String formatLocationForDependentParsingError(Template template, int line, int column) {
        return formatLocation(CustomBooleanEditor.VALUE_ON, template, line, column);
    }

    static String formatLocationForDependentParsingError(String templateSourceName, int line, int column) {
        return formatLocation(CustomBooleanEditor.VALUE_ON, templateSourceName, line, column);
    }
    ...
    private static String formatLocation(String preposition, String templateSourceName, int line, int column) {
        return formatLocation(preposition, templateSourceName, null, false, line, column);
    }
    ...
    private static String formatLocation(String preposition, String templateSourceName, String macroOrFuncName, boolean isFunction, int line, int column) {
        ...
        StringBuilder sbAppend = new StringBuilder().append("in ").append(templateDesc);
        if (macroOrFuncName != null) {
            str = " in " + (isFunction ? "function " : "macro ") + StringUtil.jQuote(macroOrFuncName);
        } else {
            str = "";
        }
        return sbAppend.append(str).append(" ").append(preposition).append(" ").append(formatPosition(line, column)).toString();
    }
    ...
    public static TemplateModelException newMethodArgUnexpectedTypeException(String methodName, int argIdx, String expectedType, TemplateModel arg) {
        return new _TemplateModelException(methodName, "(...) expects ", new _DelayedAOrAn(expectedType), " as argument #", Integer.valueOf(argIdx + 1), ", but received ", new _DelayedAOrAn(new _DelayedFTLTypeDescription(arg)), ".");
    }
    ...
    public static _TemplateModelException newCantFormatUnknownTypeDateException(Expression dateSourceExpr, UnknownDateTypeFormattingUnsupportedException cause) {
        return new _TemplateModelException(cause, (Environment) null, new _ErrorDescriptionBuilder(UNKNOWN_DATE_TO_STRING_ERROR_MESSAGE).blame(dateSourceExpr).tips(UNKNOWN_DATE_TO_STRING_TIPS));
    }
    ...
    public static TemplateException newCantFormatDateException(TemplateDateFormat format, Expression dataSrcExp, TemplateValueFormatException e, boolean useTempModelExc) {
        ...
        return useTempModelExc ? new _TemplateModelException(e, (Environment) null, desc) : new _MiscTemplateException(e, (Environment) null, desc);
    }
