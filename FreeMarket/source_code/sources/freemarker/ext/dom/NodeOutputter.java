...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/freemarker-2.3.33.jar:freemarker/ext/dom/NodeOutputter.class */
class NodeOutputter {
    ...
    private void buildPrefixLookup(Node n) {
        ...
        if (nsURI != null && nsURI.length() > 0) {
            ...
            if (prefix == null) {
                prefix = this.namespacesToPrefixLookup.get(nsURI);
                if (prefix == null) {
                    do {
                        int i = this.nextGeneratedPrefixNumber;
                        this.nextGeneratedPrefixNumber = i + 1;
                        prefix = StringUtil.toLowerABC(i);
        ...
        for (int i2 = 0; i2 < childNodes.getLength(); i2++) {
            buildPrefixLookup(childNodes.item(i2));
        }
    }
    ...
    private void constructNamespaceDecl() {
        ...
        for (String nsURI : this.namespacesToPrefixLookup.keySet()) {
            if (nsURI != null && nsURI.length() != 0) {
                String prefix = this.namespacesToPrefixLookup.get(nsURI);
                if (prefix == null) {
