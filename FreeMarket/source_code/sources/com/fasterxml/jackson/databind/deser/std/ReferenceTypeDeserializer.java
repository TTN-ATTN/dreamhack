...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/deser/std/ReferenceTypeDeserializer.class */
public abstract class ReferenceTypeDeserializer<T> extends StdDeserializer<T> implements ContextualDeserializer {
    ...
    public T deserialize(JsonParser p, DeserializationContext ctxt, T reference) throws IOException {
        ...
        if (B.equals(Boolean.FALSE) || this._valueTypeDeserializer != null) {
            if (this._valueTypeDeserializer == null) {
                objDeserializeWithType = this._valueDeserializer.deserialize(p, ctxt);
            } else {
                objDeserializeWithType = this._valueDeserializer.deserializeWithType(p, ctxt, this._valueTypeDeserializer);
            }
            contents = objDeserializeWithType;
        } else {
            Object contents2 = getReferenced(reference);
            if (contents2 == null) {
                if (this._valueTypeDeserializer == null) {
                    objDeserializeWithType2 = this._valueDeserializer.deserialize(p, ctxt);
                } else {
                    objDeserializeWithType2 = this._valueDeserializer.deserializeWithType(p, ctxt, this._valueTypeDeserializer);
                }
                Object contents3 = objDeserializeWithType2;
