...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/ser/std/ReferenceTypeSerializer.class */
public abstract class ReferenceTypeSerializer<T> extends StdSerializer<T> implements ContextualSerializer {
    ...
    public JsonSerializer<?> createContextual(SerializerProvider provider, BeanProperty property) throws JsonMappingException {
        ReferenceTypeSerializer<T> referenceTypeSerializerWithResolved;
        ...
        if (this._property == property && this._valueTypeSerializer == typeSer && this._valueSerializer == ser) {
            referenceTypeSerializerWithResolved = this;
        } else {
            referenceTypeSerializerWithResolved = withResolved(property, typeSer, ser, this._unwrapper);
        }
        if (property != null && (inclV = property.findPropertyInclusion(provider.getConfig(), handledType())) != null && (incl = inclV.getContentInclusion()) != JsonInclude.Include.USE_DEFAULTS) {
            ...
            if (this._suppressableValue != valueToSuppress || this._suppressNulls != suppressNulls) {
                referenceTypeSerializerWithResolved = referenceTypeSerializerWithResolved.withContentInclusion(valueToSuppress, suppressNulls);
            }
        }
    ...
    @Override // com.fasterxml.jackson.databind.JsonSerializer
    public boolean isEmpty(SerializerProvider provider, T value) throws IllegalArgumentException {
    ...
    public void serialize(T ref, JsonGenerator g, SerializerProvider provider) throws IOException, IllegalArgumentException {
    ...
    @Override // com.fasterxml.jackson.databind.JsonSerializer
    public void serializeWithType(T ref, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer) throws IOException, IllegalArgumentException {
        Object value = _getReferencedIfPresent(ref);
        if (value == null) {
            if (this._unwrapper == null) {
                provider.defaultSerializeNull(g);
    ...
    private final JsonSerializer<Object> _findCachedSerializer(SerializerProvider provider, Class<?> rawType) throws JsonMappingException, IllegalArgumentException {
