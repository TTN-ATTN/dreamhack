...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/ser/DefaultSerializerProvider.class */
public abstract class DefaultSerializerProvider extends SerializerProvider implements Serializable {
    ...
    @Override // com.fasterxml.jackson.databind.SerializerProvider
    public Object includeFilterInstance(BeanPropertyDefinition forProperty, Class<?> filterClass) throws NoSuchMethodException, SecurityException, IllegalArgumentException {
        if (filterClass == null) {
            return null;
        }
        HandlerInstantiator hi = this._config.getHandlerInstantiator();
    ...
    public void serializeValue(JsonGenerator gen, Object value, JavaType rootType, JsonSerializer<Object> ser) throws IOException {
        ...
        PropertyName rootName2 = this._config.getFullRootName();
        if (rootName2 == null) {
            if (this._config.isEnabled(SerializationFeature.WRAP_ROOT_VALUE)) {
                if (rootType == null) {
                    rootName = this._config.findRootName(value.getClass());
                } else {
                    rootName = this._config.findRootName(rootType);
                }
                _serialize(gen, value, ser, rootName);
                return;
            }
        } else if (!rootName2.isEmpty()) {
            _serialize(gen, value, ser, rootName2);
            return;
        }
        _serialize(gen, value, ser);
    }
    ...
    public JsonSchema generateJsonSchema(Class<?> type) throws JsonMappingException {
        JsonFormatVisitable jsonFormatVisitableFindValueSerializer = findValueSerializer(type, (BeanProperty) null);
        JsonNode schemaNode = jsonFormatVisitableFindValueSerializer instanceof SchemaAware ? ((SchemaAware) jsonFormatVisitableFindValueSerializer).getSchema(this, null) : JsonSchema.getDefaultSchemaNode();
    ...
    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/ser/DefaultSerializerProvider$Impl.class */
