...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/introspect/AnnotatedMethodCollector.class */
public class AnnotatedMethodCollector extends CollectorBase {
    ...
    AnnotatedMethodMap collect(TypeFactory typeFactory, TypeResolutionContext tc, JavaType mainType, List<JavaType> superTypes, Class<?> primaryMixIn) throws NoSuchMethodException, SecurityException, ClassNotFoundException {
        Class<?> mixin;
    ...
    private void _addMemberMethods(TypeResolutionContext tc, Class<?> cls, Map<MemberKey, MethodBuilder> methods, Class<?> mixInCls) throws SecurityException, ClassNotFoundException {
        if (mixInCls != null) {
    ...
    protected void _addMethodMixIns(TypeResolutionContext tc, Class<?> targetClass, Map<MemberKey, MethodBuilder> methods, Class<?> mixInCls) throws SecurityException {
        if (this._intr == null) {
            return;
        }
    ...
    private static boolean _isIncludableMemberMethod(Method m) {
        return (Modifier.isStatic(m.getModifiers()) || m.isSynthetic() || m.isBridge() || m.getParameterTypes().length > 2) ? false : true;
    }

    /* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-databind-2.13.5.jar:com/fasterxml/jackson/databind/introspect/AnnotatedMethodCollector$MethodBuilder.class */
