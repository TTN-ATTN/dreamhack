...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-core-2.13.5.jar:com/fasterxml/jackson/core/json/UTF8DataInputJsonParser.class */
public class UTF8DataInputJsonParser extends ParserBase {
    ...
    public int readBinaryValue(Base64Variant b64variant, OutputStream out) throws IOException {
        ...
        try {
            int i_readBinary = _readBinary(b64variant, out, buf);
    ...
    /* JADX WARN: Code restructure failed: missing block: B:38:0x0131, code lost:
        ...
        throw reportInvalidBase64Char(r8, r0, 3, "expected padding character '" + r8.getPaddingChar() + "'");
     */
    /*
    ...
    */
    ...
    protected final JsonToken _parseFloatThatStartsWithPeriod() throws IOException {
        ...
        return _parseFloat(outBuf, 0, 46, false, 0);
    }
    ...
    protected final String parseEscapedName(int[] quads, int qlen, int currQuad, int ch2, int currQuadBytes) throws IOException {
        ...
        while (true) {
                ...
                if (ch2 > 127) {
                    if (currQuadBytes >= 4) {
                        if (qlen >= quads.length) {
                            int[] iArr_growArrayBy = _growArrayBy(quads, quads.length);
                            quads = iArr_growArrayBy;
                    ...
                    if (ch2 < 2048) {
                        ...
                        if (currQuadBytes2 >= 4) {
                            if (qlen >= quads.length) {
                                int[] iArr_growArrayBy2 = _growArrayBy(quads, quads.length);
                                quads = iArr_growArrayBy2;
            ...
            if (currQuadBytes < 4) {
                ...
                if (qlen >= quads.length) {
                    int[] iArr_growArrayBy3 = _growArrayBy(quads, quads.length);
                    quads = iArr_growArrayBy3;
        ...
        if (currQuadBytes > 0) {
            if (qlen >= quads.length) {
                int[] iArr_growArrayBy4 = _growArrayBy(quads, quads.length);
                quads = iArr_growArrayBy4;
                this._quadBuffer = iArr_growArrayBy4;
            }
            int i4 = qlen;
            qlen++;
    ...
    protected String _handleOddName(int ch2) throws IOException {
        if (ch2 == 39 && (this._features & FEAT_MASK_ALLOW_SINGLE_QUOTES) != 0) {
    ...
    private final String addName(int[] quads, int qlen, int lastQuadBytes) throws JsonParseException {
        ...
        while (ix < byteLen) {
            int byteIx = ix & 3;
            int ch3 = (quads[ix >> 2] >> ((3 - byteIx) << 3)) & Const.MAX_ARRAY_DIMENSIONS;
            ix++;
            if (ch3 > 127) {
                if ((ch3 & 224) == 192) {
                    ch2 = ch3 & 31;
                ...
                if (needed > 1) {
                    ...
                    if (needed > 2) {
                        ...
                        if ((ch24 & Opcodes.CHECKCAST) != 128) {
                            _reportInvalidOther(ch24 & Const.MAX_ARRAY_DIMENSIONS);
                        }
                        ch3 = (ch3 << 6) | (ch24 & 63);
                    }
                }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:5:0x004b, code lost:
        ...
        if (r5._parsingContext.inArray() == false) goto L30;
     */
    ...
    */
    ...
    protected int _decodeCharForError(int firstByte) throws IOException {
        int needed;
        int c = firstByte & Const.MAX_ARRAY_DIMENSIONS;
        if (c > 127) {
            if ((c & 224) == 192) {
                c &= 31;
                needed = 1;
                ...
                _reportInvalidInitial(c & Const.MAX_ARRAY_DIMENSIONS);
                needed = 1;
            }
            int d = this._inputData.readUnsignedByte();
            if ((d & Opcodes.CHECKCAST) != 128) {
                _reportInvalidOther(d & Const.MAX_ARRAY_DIMENSIONS);
            }
            c = (c << 6) | (d & 63);
            if (needed > 1) {
                int d2 = this._inputData.readUnsignedByte();
                if ((d2 & Opcodes.CHECKCAST) != 128) {
                    _reportInvalidOther(d2 & Const.MAX_ARRAY_DIMENSIONS);
                }
                c = (c << 6) | (d2 & 63);
                if (needed > 2) {
                    int d3 = this._inputData.readUnsignedByte();
                    if ((d3 & Opcodes.CHECKCAST) != 128) {
                        _reportInvalidOther(d3 & Const.MAX_ARRAY_DIMENSIONS);
                    }
                    c = (c << 6) | (d3 & 63);
                }
            }
    ...
    private final void _skipUtf8_2() throws IOException {
        ...
        if ((c & Opcodes.CHECKCAST) != 128) {
            _reportInvalidOther(c & Const.MAX_ARRAY_DIMENSIONS);
        }
    }
    ...
    private final void _skipUtf8_3() throws IOException {
        ...
        if ((c2 & Opcodes.CHECKCAST) != 128) {
            _reportInvalidOther(c2 & Const.MAX_ARRAY_DIMENSIONS);
        }
    }
    ...
    protected void _reportInvalidToken(int ch2, String matchedPart) throws IOException {
        _reportInvalidToken(ch2, matchedPart, _validJsonTokenList());
    }
    ...
    /*
        ...
        To view partially-correct add '--show-bad-code' argument
    */
    protected final byte[] _decodeBase64(com.fasterxml.jackson.core.Base64Variant r8) throws java.io.IOException {
        /*
