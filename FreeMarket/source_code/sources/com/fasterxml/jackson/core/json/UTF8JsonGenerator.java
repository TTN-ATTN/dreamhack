...
/* loaded from: free-market-1.0.0.jar:BOOT-INF/lib/jackson-core-2.13.5.jar:com/fasterxml/jackson/core/json/UTF8JsonGenerator.class */
public class UTF8JsonGenerator extends JsonGeneratorImpl {
            ...
            do {
                ...
                if (c <= 127) {
                    ...
                    if (ch2 < 2048) {
                        ...
                        int i3 = this._outputTail;
                        this._outputTail = i3 + 1;
                        bArr2[i3] = (byte) (192 | (ch2 >> 6));
                        byte[] bArr3 = this._outputBuffer;
                        int i4 = this._outputTail;
                        this._outputTail = i4 + 1;
                        bArr3[i4] = (byte) (128 | (ch2 & '?'));
                    } else {
                        offset = _outputRawMultiByteChar(ch2, cbuf, offset, len2);
                    }
                }
            } while (offset < len2);
            return;
            ...
            do {
                ...
                if (c < 128) {
                    ...
                    if (ch2 < 2048) {
                        int i3 = this._outputTail;
                        this._outputTail = i3 + 1;
                        bbuf[i3] = (byte) (192 | (ch2 >> 6));
                        int i4 = this._outputTail;
                        this._outputTail = i4 + 1;
                        bbuf[i4] = (byte) (128 | (ch2 & '?'));
                    } else {
                        offset = _outputRawMultiByteChar(ch2, cbuf, offset, inputEnd);
                    }
                }
            } while (offset < inputEnd);
            return;
            ...
            do {
                ...
                if (c <= 127) {
                    ...
                    if (ch2 < 2048) {
                        ...
                        offset = _outputRawMultiByteChar(ch2, cbuf, offset, end);
                    }
                }
            } while (offset < end);
            return;
    ...
    private final int _handleLongCustomEscape(byte[] outputBuffer, int outputPtr, int outputEnd, byte[] raw, int remainingChars) throws IOException {
        ...
        System.arraycopy(raw, 0, outputBuffer, outputPtr, len);
        int outputPtr2 = outputPtr + len;
        if (outputPtr2 + (6 * remainingChars) > outputEnd) {
    ...
    private int _writeGenericEscape(int charToEscape, int outputPtr) throws IOException {
        ...
        bbuf[outputPtr3] = 117;
        if (charToEscape > 255) {
            int hi = (charToEscape >> 8) & Const.MAX_ARRAY_DIMENSIONS;
            int outputPtr5 = outputPtr4 + 1;
            bbuf[outputPtr4] = HEX_CHARS[hi >> 4];
            outputPtr2 = outputPtr5 + 1;
            bbuf[outputPtr5] = HEX_CHARS[hi & 15];
            charToEscape &= Const.MAX_ARRAY_DIMENSIONS;
        } else {
