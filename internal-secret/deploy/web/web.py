...
app = Flask(__name__)
JOB_DB = 'jobs.sqlite3'
ALLOWED_HOST = "example.com" # only this host is allowed
...
def init_db():
    if not os.path.exists(JOB_DB):
        ...
        cur = conn.cursor()
        cur.execute("CREATE TABLE jobs (id TEXT PRIMARY KEY, url TEXT, status TEXT, result TEXT, created REAL)") # result holds the flag
...
def host_allowed(parsed):
    ...
    return host == ALLOWED_HOST # bypass: http://example.com#@redirector:8081/...
...
def worker_process(job_id):
    ...
    try:
        resp = requests.get(url, timeout=6, allow_redirects=True) # target sink: use http://username:password@host/path to bypass 
        snippet = resp.text[:1000].replace('\n',' ')
        result = f"code={resp.status_code} headers={dict(resp.headers)} body_snippet={snippet}" # body_snippet holds the flag
        cur.execute("UPDATE jobs SET status=?, result=? WHERE id=?", ('done', result, job_id)) # the flag is stored in result
        conn.commit()
...
def fetch():
    ...
    try:
        parsed = urlparse(unquote(url)) # unquote just to check
    except Exception:
        return jsonify({"ok": False, "reason": "bad url"}), 400
    if not host_allowed(parsed):
        return jsonify({"ok": False, "reason": "host not allowed"}), 403
    job_id = enqueue_job(url, client_ip) # vul: parse the original URL 
    return jsonify({"ok": True, "job_id": job_id}), 200 # the job id holds the flag
...
def result(job_id):
    ...
    # must have admin token, CANNOT bypass because there's no sink
    ...
    return jsonify({"id": row[0], "url": row[1], "status": row[2], "result": row[3], "created": row[4]}) # flag cannot be retrieved here
...
def audit():
    ...
    conn = sqlite3.connect(JOB_DB)
    cur = conn.cursor()
    cur.execute(f"SELECT ev, job, info, t FROM audit ORDER BY {order} DESC LIMIT 80") # vul: Blind SQLi 
    # SQLi based on t because job_id is needed to bruteforce the flag, which means the job_id of returned json blocks is the same
    rows = cur.fetchall()
    conn.close()
    events = []
    for r in rows[::-1]:
